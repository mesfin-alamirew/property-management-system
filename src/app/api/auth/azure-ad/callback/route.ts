...
export async function GET(request: Request) {
  ...
  try {
    console.log('[Azure AD callback] Step 1: authenticating Azure AD code');

    const identity = await authenticateAzureAd(code);

    console.log('[Azure AD callback] Step 2: resolving authenticated user');

    const user = await resolveAuthenticatedUser(identity);

    console.log('[Azure AD callback] Step 3: starting user session');

    await startUserSession(user.id);

    console.log('[Azure AD callback] Step 4: checking active role');

    const hasRole = await userHasActiveRole(user.id);

    console.log(
      `[Azure AD callback] Step 5: authentication completed; hasRole=${hasRole}`,
    );
    ...
    console.error('[Azure AD callback] Authentication failed');
